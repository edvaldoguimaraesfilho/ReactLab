...
App 10 is **“Lista de Emails / Email List”** in Block 1. In the project roadmap, it is defined as an **Outlook-style email simulation** focused on **corporate layout**. 

React’s official docs recommend rendering collections with `map()` and stable keys, which is exactly the main concept in this app. ([React][1]) Fluent UI is appropriate here because Microsoft positions it for building apps with accessibility, internationalization, and performance included. ([Microsoft Developer][2])
...
```powershell
New-Item src\models\EmailMessage.ts -ItemType File
New-Item src\data\emails.ts -ItemType File
New-Item src\components\EmailItem.tsx -ItemType File
New-Item src\components\EmailList.tsx -ItemType File
```

---

## 2. `src\models\EmailMessage.ts`
...
## 4. `src\components\EmailItem.tsx`
...
## 5. `src\components\EmailList.tsx`
...
| ------------------ | ----------------------------------- |
| TypeScript model   | `EmailMessage.ts`                   |
| Static data source | `emails.ts`                         |
| Reusable component | `EmailItem.tsx`                     |
| List rendering     | `EmailList.tsx`                     |
| `map()`            | `emails.map(...)`                   |
| Stable key         | `key={email.id}`                    |
| Props              | `email` passed into `EmailItem`     |
| Fluent UI layout   | `Card`, `Avatar`, `Badge`, `Button` |
| Corporate UI       | Outlook-style inbox layout          |
